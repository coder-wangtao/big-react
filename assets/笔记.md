...
reconcile
拿当前 dom 或者组件的 FiberNode 的子 ReactElement 与 原来的子 FiberNode 做比较
current 的 child 和 workInProgress 的 children(dom/组件) 作比较
挂载时：current 的 child 为 null,生成新的 children(dom/组件) FiberNode

更新时：current 的 child 不为 null，判断 workInProgress 的 children、current 的 child 是不是相同（能不能复用），
能复用： 生成新的 children(dom/组件) FiberNode，形成双指针。
不能复用：给当前的 workInProgress 加上 deletions = 要被删除的 FiberNode，并且打上标记 Flags = ChildDeletion,
然後再根据对应的 workInProgress 的 children type 类型创建新的 FiberNode,新的 FiberNode 打上标记 Flags = Placement,
