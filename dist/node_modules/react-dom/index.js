(function (global, factory) {
  ...
  const HostText = 6; //<div>123</div> 下的123

  const NoFlags = 0b0000000;
  const Placement = 0b0000001; //跟结构相关
  const Update = 0b0000010; //跟属性相关
  const ChildDeletion = 0b0000100; //跟结构相关
  ...
  class FiberNode {
      ...
      index; // 索引
      // 作为工作单元
      pendingProps; //刚开始的props
      memoizedProps; // 已经更新完的props
      memoizedState; // 更新完成后新的 State
      alternate; //指向节点的备份节点，用于在协调过程中进行比较 (双缓存)
  ...
  let workInProgress; //workInProgress在global作用域，类似链表的指针，标记当前处理元素的位置
